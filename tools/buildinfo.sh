...
#Add Begin by shaopan.tang 2023-01-12 [FP5-176]IMS user agent
datetime=`awk -F\- '{print $2}' version/build_datetime.h`
display_version=`awk -F\" '/BUILD_AP_VER/{print $2}' version/build_version.h`

if [ "$TARGET_BUILD_VARIANT" = "user" ]; then
    BUILD_DISPLAY_ID=FP5.$BUILD_NUMBER.$display_version.$datetime
else
    BUILD_DISPLAY_ID=FP5.$BUILD_NUMBER.$display_version-userdebug.$datetime
fi
...
echo "ro.build.version.base_os=Fairphone/FP5/FP5:13/TKQ1.230127.002/TT4H:user/release-keys"
